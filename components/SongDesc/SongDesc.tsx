import React from "react";

export default function SongDesc({ song }) {
    return (
        <div className="uk-section uk-section-default">
            <div className="uk-container uk-container-large">
                <div className="uk-grid-large" data-uk-grid>
                    <div className="uk-width-expand@m">
                        <div className="uk-article">
                            <hr
                                className="
                                    uk-margin-medium-top uk-margin-large-bottom
                                "
                            />
                            <h3 className="uk-heading-small">Comments</h3>
                            <ul className="uk-comment-list uk-margin-medium-top">
                                <li>
                                    <article
                                        className="uk-comment uk-visible-toggle"
                                        tabIndex={-1}
                                    >
                                        <header
                                            className="
                                                uk-comment-header
                                                uk-position-relative
                                            "
                                        >
                                            <div
                                                className="
                                                    uk-grid-medium
                                                    uk-flex-middle
                                                "
                                                data-uk-grid
                                            >
                                                <div className="uk-width-auto">
                                                    <img
                                                        className="
                                                            uk-comment-avatar
                                                            uk-border-circle
                                                        "
                                                        src="https://via.placeholder.com/100x100"
                                                        width="50"
                                                        height="50"
                                                        alt="Muhannad"
                                                    />
                                                </div>
                                                <div className="uk-width-expand">
                                                    <h4
                                                        className="
                                                            uk-comment-title
                                                            uk-margin-remove
                                                        "
                                                    >
                                                        <a
                                                            className="
                                                                uk-link-reset
                                                            "
                                                            href="#"
                                                        >
                                                            Muhannad
                                                        </a>
                                                    </h4>
                                                    <p
                                                        className="
                                                            uk-comment-meta
                                                            uk-margin-remove
                                                        "
                                                    >
                                                        <a
                                                            className="
                                                                uk-link-reset
                                                            "
                                                            href="#"
                                                        >
                                                            12 days ago
                                                        </a>
                                                    </p>
                                                    <div className="uk-rating">
                                                        <span
                                                            className="
                                                                uk-rating-filled
                                                            "
                                                            data-uk-icon="icon: star; ratio: 0.8"
                                                        ></span>
                                                        <span
                                                            className="
                                                                uk-rating-filled
                                                            "
                                                            data-uk-icon="icon: star; ratio: 0.8"
                                                        ></span>
                                                        <span
                                                            className="
                                                                uk-rating-filled
                                                            "
                                                            data-uk-icon="icon: star; ratio: 0.8"
                                                        ></span>
                                                        <span
                                                            className="
                                                                uk-rating-filled
                                                            "
                                                            data-uk-icon="icon: star; ratio: 0.8"
                                                        ></span>
                                                        <span data-uk-icon="icon: star; ratio: 0.8"></span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div
                                                className="
                                                    uk-position-top-right
                                                    uk-position-small
                                                    uk-hidden-hover
                                                "
                                            >
                                                <a
                                                    className="uk-link-muted"
                                                    href="#"
                                                >
                                                    Reply
                                                </a>
                                            </div>
                                        </header>
                                        <div className="uk-comment-body">
                                            <p>
                                                Edgy and driving, with indie
                                                rock elements featuring
                                                confident electric gutiar and
                                                male aahs to createa masculine
                                                and empowering mood.
                                            </p>
                                        </div>
                                    </article>
                                    <ul>
                                        <li>
                                            <article
                                                className="
                                                    uk-comment
                                                    uk-comment-primary
                                                    uk-visible-toggle
                                                    uk-border-rounded
                                                "
                                                tabIndex={-1}
                                            >
                                                <header
                                                    className="
                                                        uk-comment-header
                                                        uk-position-relative
                                                    "
                                                >
                                                    <div
                                                        className="
                                                            uk-grid-medium
                                                            uk-flex-middle
                                                        "
                                                        data-uk-grid
                                                    >
                                                        <div
                                                            className="
                                                                uk-width-auto
                                                            "
                                                        >
                                                            <img
                                                                className="
                                                                    uk-comment-avatar
                                                                    uk-border-circle
                                                                "
                                                                src="https://via.placeholder.com/100x100"
                                                                width="50"
                                                                height="50"
                                                                alt="Muhannad Eyno"
                                                            />
                                                        </div>
                                                        <div
                                                            className="
                                                                uk-width-expand
                                                            "
                                                        >
                                                            <h4
                                                                className="
                                                                    uk-comment-title
                                                                    uk-margin-remove
                                                                "
                                                            >
                                                                <a
                                                                    className="
                                                                        uk-link-reset
                                                                    "
                                                                    href="#"
                                                                >
                                                                    Muhannad
                                                                    Eyno
                                                                </a>
                                                            </h4>
                                                            <p
                                                                className="
                                                                    uk-comment-meta
                                                                    uk-margin-remove-top
                                                                "
                                                            >
                                                                <a
                                                                    className="
                                                                        uk-link-reset
                                                                    "
                                                                    href="#"
                                                                >
                                                                    12 days ago
                                                                </a>
                                                            </p>
                                                        </div>
                                                    </div>
                                                    <div
                                                        className="
                                                            uk-position-top-right
                                                            uk-position-small
                                                            uk-hidden-hover
                                                        "
                                                    >
                                                        <a
                                                            className="
                                                                uk-link-muted
                                                            "
                                                            href="#"
                                                        >
                                                            Reply
                                                        </a>
                                                    </div>
                                                </header>
                                                <div className="uk-comment-body">
                                                    <p>
                                                        Edgy and driving, with
                                                        indie rock elements
                                                        featuring confident
                                                        electric gutiar and male
                                                        aahs to createa
                                                        masculine and empowering
                                                        mood et justo duo
                                                        dolores et ea rebum.
                                                        Stet clita kasd
                                                        gubergren, no sea
                                                        takimata sanctus est
                                                        Lorem ipsum dolor sit
                                                        amet.
                                                    </p>
                                                </div>
                                            </article>
                                        </li>
                                        <li>
                                            <article
                                                className="
                                                    uk-comment uk-visible-toggle
                                                "
                                                tabIndex={-1}
                                            >
                                                <header
                                                    className="
                                                        uk-comment-header
                                                        uk-position-relative
                                                    "
                                                >
                                                    <div
                                                        className="
                                                            uk-grid-medium
                                                            uk-flex-middle
                                                        "
                                                        data-uk-grid
                                                    >
                                                        <div
                                                            className="
                                                                uk-width-auto
                                                            "
                                                        >
                                                            <img
                                                                className="
                                                                    uk-comment-avatar
                                                                    uk-border-circle
                                                                "
                                                                src="https://via.placeholder.com/100x100"
                                                                width="50"
                                                                height="50"
                                                                alt="Muhannad"
                                                            />
                                                        </div>
                                                        <div
                                                            className="
                                                                uk-width-expand
                                                            "
                                                        >
                                                            <h4
                                                                className="
                                                                    uk-comment-title
                                                                    uk-margin-remove
                                                                "
                                                            >
                                                                <a
                                                                    className="
                                                                        uk-link-reset
                                                                    "
                                                                    href="#"
                                                                >
                                                                    Muhannad
                                                                </a>
                                                            </h4>
                                                            <p
                                                                className="
                                                                    uk-comment-meta
                                                                    uk-margin-remove
                                                                "
                                                            >
                                                                <a
                                                                    className="
                                                                        uk-link-reset
                                                                    "
                                                                    href="#"
                                                                >
                                                                    12 days ago
                                                                </a>
                                                            </p>
                                                            <div
                                                                className="
                                                                    uk-rating
                                                                "
                                                            >
                                                                <span
                                                                    className="
                                                                        uk-rating-filled
                                                                    "
                                                                    data-uk-icon="icon: star; ratio: 0.8"
                                                                ></span>
                                                                <span
                                                                    className="
                                                                        uk-rating-filled
                                                                    "
                                                                    data-uk-icon="icon: star; ratio: 0.8"
                                                                ></span>
                                                                <span
                                                                    className="
                                                                        uk-rating-filled
                                                                    "
                                                                    data-uk-icon="icon: star; ratio: 0.8"
                                                                ></span>
                                                                <span
                                                                    className="
                                                                        uk-rating-filled
                                                                    "
                                                                    data-uk-icon="icon: star; ratio: 0.8"
                                                                ></span>
                                                                <span data-uk-icon="icon: star; ratio: 0.8"></span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div
                                                        className="
                                                            uk-position-top-right
                                                            uk-position-small
                                                            uk-hidden-hover
                                                        "
                                                    >
                                                        <a
                                                            className="
                                                                uk-link-muted
                                                            "
                                                            href="#"
                                                        >
                                                            Reply
                                                        </a>
                                                    </div>
                                                </header>
                                                <div className="uk-comment-body">
                                                    <p>
                                                        Edgy and driving, with
                                                        indie rock elements
                                                        featuring confident
                                                        electric gutiar and male
                                                        aahs to createa
                                                        masculine and empowering
                                                        mood
                                                    </p>
                                                </div>
                                            </article>
                                        </li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div className="uk-width-1-3@m">
                        <h3 className="uk-heading-small">Related tracks</h3>
                        <ul
                            className="
                                uk-list
                                uk-list-large
                                uk-list-divider
                                uk-margin-medium-top
                            "
                        >
                            <li>
                                <a
                                    href="#"
                                    className="display-bl uk-animation-toggle"
                                    tabIndex={0}
                                >
                                    <span
                                        data-uk-icon="icon: play-circle; ratio:1"
                                        className="
                                            uk-icon
                                            icon-music
                                            uk-animation-slide-left-small
                                        "
                                    ></span>
                                    <h3
                                        className="
                                            uk-card-title
                                            uk-text-500
                                            uk-margin-small-bottom
                                            uk-margin-small-top
                                            display-inline
                                            uk-animation-slide-left-small
                                        "
                                    >
                                        Birth of a Star
                                    </h3>
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#"
                                    className="display-bl uk-animation-toggle"
                                    tabIndex={0}
                                >
                                    <span
                                        data-uk-icon="icon: play-circle; ratio:1"
                                        className="
                                            uk-icon
                                            icon-music
                                            uk-animation-slide-left-small
                                        "
                                    ></span>
                                    <h3
                                        className="
                                            uk-card-title
                                            uk-text-500
                                            uk-margin-small-bottom
                                            uk-margin-small-top
                                            display-inline
                                            uk-animation-slide-left-small
                                        "
                                    >
                                        They Need Us
                                    </h3>
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#"
                                    className="display-bl uk-animation-toggle"
                                    tabIndex={0}
                                >
                                    <span
                                        data-uk-icon="icon: play-circle; ratio:1"
                                        className="
                                            uk-icon
                                            icon-music
                                            uk-animation-slide-left-small
                                        "
                                    ></span>
                                    <h3
                                        className="
                                            uk-card-title
                                            uk-text-500
                                            uk-margin-small-bottom
                                            uk-margin-small-top
                                            display-inline
                                            uk-animation-slide-left-small
                                        "
                                    >
                                        Poboy
                                    </h3>
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#"
                                    className="display-bl uk-animation-toggle"
                                    tabIndex={0}
                                >
                                    <span
                                        data-uk-icon="icon: play-circle; ratio:1"
                                        className="
                                            uk-icon
                                            icon-music
                                            uk-animation-slide-left-small
                                        "
                                    ></span>
                                    <h3
                                        className="
                                            uk-card-title
                                            uk-text-500
                                            uk-margin-small-bottom
                                            uk-margin-small-top
                                            display-inline
                                            uk-animation-slide-left-small
                                        "
                                    >
                                        August Heat
                                    </h3>
                                </a>
                            </li>
                        </ul>
                        <h3 className="uk-margin-large-top uk-heading-small">
                            Track keywords
                        </h3>
                        <div className="uk-margin-medium-top" data-uk-margin>
                            <a className="uk-display-inline-block" href="#">
                                <span className="uk-label uk-label-light">
                                    Genres
                                </span>
                            </a>
                            <a className="uk-display-inline-block" href="#">
                                <span className="uk-label uk-label-light">
                                    Rock
                                </span>
                            </a>
                            <a className="uk-display-inline-block" href="#">
                                <span className="uk-label uk-label-light">
                                    Arousing
                                </span>
                            </a>
                            <a className="uk-display-inline-block" href="#">
                                <span className="uk-label uk-label-light">
                                    Energetic
                                </span>
                            </a>
                            <a className="uk-display-inline-block" href="#">
                                <span className="uk-label uk-label-light">
                                    Male aahs
                                </span>
                            </a>
                        </div>
                        <h3 className="uk-margin-large-top uk-heading-small">
                            Share Track
                        </h3>
                        <div className="uk-margin-medium-top">
                            <div
                                data-uk-grid
                                className="uk-child-width-auto uk-grid-small"
                            >
                                <div>
                                    <a
                                        href="#"
                                        data-uk-icon="icon: facebook; ratio: 0.9"
                                        className="uk-icon-button facebook"
                                        target="_blank"
                                    ></a>
                                </div>
                                <div>
                                    <a
                                        href="#"
                                        data-uk-icon="icon: linkedin; ratio: 0.9"
                                        className="uk-icon-button linkedin"
                                        target="_blank"
                                    ></a>
                                </div>
                                <div>
                                    <a
                                        href="#"
                                        data-uk-icon="icon: twitter; ratio: 0.9"
                                        className="uk-icon-button twitter"
                                        target="_blank"
                                    ></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
}
